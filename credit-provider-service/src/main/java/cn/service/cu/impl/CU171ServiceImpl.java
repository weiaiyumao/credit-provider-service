...
import cn.repository.cu.CU171Repository;
import cn.service.cu.CU171Service;

@Service
public class CU171ServiceImpl implements CU171Service {

	@Autowired
	private CU171Repository repository;
	
	@Autowired
	private MongoTemplate mongoTemplate;
	
	@Override
	public List<CU171> findByMobileAndReportTime(String mobile, Date startTime, Date endTime) {
		Sort sort = new Sort(Direction.DESC,"reportTime");
		Pageable pageable = new PageRequest(1 - 1, 1, sort);
		Page<CU171> page  = repository.findByMobileAndReportTime(mobile,startTime,endTime,pageable);
		return page.getContent();
	}

	@Override
	public List<CU171> findByMobile(String mobile) {
	...
	public void deleteByMobile(BaseMobileDetail mobileDetail,String mobile) {
		List<CU171> resultList = this.findByMobile(mobile);
