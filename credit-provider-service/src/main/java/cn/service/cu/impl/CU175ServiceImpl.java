...
import cn.repository.cu.CU175Repository;
import cn.service.cu.CU175Service;

@Service
public class CU175ServiceImpl implements CU175Service {

	@Autowired
	private CU175Repository repository;
	
	@Autowired
	private MongoTemplate mongoTemplate;
	
	@Override
	public List<CU175> findByMobileAndReportTime(String mobile, Date startTime, Date endTime) {
		Sort sort = new Sort(Direction.DESC,"reportTime");
		Pageable pageable = new PageRequest(1 - 1, 1, sort);
		Page<CU175> page  = repository.findByMobileAndReportTime(mobile,startTime,endTime,pageable);
		return page.getContent();
	}

	@Override
	public List<CU175> findByMobile(String mobile) {
	...
	public void deleteByMobile(BaseMobileDetail mobileDetail,String mobile) {
		List<CU175> resultList = this.findByMobile(mobile);
